...
fn test_before_context_at_start_of_file() {
    ...
    assert_eq!(
        lines.len(),
        1,
        "Should only show match (no lines before it)"
    );
    assert!(result.output.contains("match"));
}
...
fn test_overlapping_context_regions() {
    ...
    assert!(
        lines.len() >= 4,
        "Should show both matches with their contexts"
    );
}
...
fn test_large_before_context() {
    ...
    assert_eq!(
        lines.len(),
        3,
        "Should show all available lines before match"
    );
}
...
fn test_large_after_context() {
    ...
    assert_eq!(
        lines.len(),
        3,
        "Should show all available lines after match"
    );
}
