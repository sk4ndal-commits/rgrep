...
fn test_simple_and_pattern() {
    ...
    assert_eq!(lines.len(), 2, "Should match exactly 2 lines with both foo and bar");
}
...
fn test_case_insensitive_with_or() {
    ...
    let lines: Vec<&str> = result.output.lines().collect();
    assert_eq!(lines.len(), 4, "Should match all case variations of foo or bar");
}
...
fn test_case_insensitive_with_and() {
    ...
    let lines: Vec<&str> = result.output.lines().collect();
    assert_eq!(lines.len(), 3, "Should match all lines with both words regardless of case");
}
...
fn test_multiple_and_operators() {
    ...
    let lines: Vec<&str> = result.output.lines().collect();
    assert_eq!(lines.len(), 1, "Should match only line with all three terms");
}
...
fn test_multiple_or_operators() {
    ...
    let lines: Vec<&str> = result.output.lines().collect();
    assert_eq!(lines.len(), 3, "Should match lines with any of the three terms");
}
...
fn test_boolean_with_spaces_in_patterns() {
    ...
    let lines: Vec<&str> = result.output.lines().collect();
    assert_eq!(lines.len(), 2, "Should match lines with 'hello world' or 'foo bar'");
}
